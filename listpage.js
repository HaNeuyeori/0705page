...
function printPagination({prev, start, end, next}, pageno, $parent) {
  if(prev>0) {
    const html =`
      <li class='page-item'>
        <a class='page-link' href='INCAlistpage.html?pageno=${prev}'>이전으로</a>
      </li>`;
    $parent.append(html);
  }
  for(let i=start; i<=end; i++) {
    ...
    const html =`
      <li class='${className}'>
        <a class='page-link' href='INCAlistpage.html?pageno=${i}'>${i}</a>
      </li>`;
    $parent.append(html);  
  }
  if(next>0) {
    const html =`
      <li class='page-item'>
        <a class='page-link' href='INCAlistpage.html?pageno=${next}'>다음으로</a>
      </li>`;
    $parent.append(html);
  }
}